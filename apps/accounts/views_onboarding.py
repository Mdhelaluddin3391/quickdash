    ...
    def post(self, request):
        ...
        if not created:
            if profile.approval_status == RiderProfile.ApprovalStatus.APPROVED:
                return Response({"detail": "You are already an approved rider.", "status": profile.approval_status}, status=status.HTTP_200_OK)
            if vehicle_type:
                profile.vehicle_type = vehicle_type
            profile.save()

        return Response({"detail": "Rider application submitted. Admin will review.", "approval_status": profile.approval_status}, status=status.HTTP_200_OK)
...
class AdminApproveRiderView(views.APIView):
    ...
    def post(self, request, pk):
        ...
        return Response({"detail": "Rider approved.", "status": profile.status}, status=status.HTTP_200_OK)
...
class AdminRejectRiderView(views.APIView):
    ...
    def post(self, request, pk):
        ...
        profile.save(update_fields=["approval_status", "status"])
        return Response({"detail": "Rider rejected."}, status=status.HTTP_200_OK)
...
class AdminEmployeeListCreateView(views.APIView):
    ...
    def post(self, request):
        ...
        if hasattr(user, "employee_profile"):
            return Response({"detail": "Employee already exists for this phone."}, status=status.HTTP_400_BAD_REQUEST)

        profile = EmployeeProfile.objects.create(user=user, employee_code=employee_code, role=role, warehouse_code=warehouse_code)
...
class AdminEmployeeStatusUpdateView(views.APIView):
    ...
    def post(self, request, pk):
        ...
        if status_str == "ACTIVE":
            ...
            return Response({"detail": "Invalid status. Use 'ACTIVE' or 'INACTIVE'."}, status=status.HTTP_400_BAD_REQUEST)

        profile.save(update_fields=["is_active_employee"])
        return Response({"detail": "Employee status updated.", "is_active_employee": profile.is_active_employee}, status=status.HTTP_200_OK)
