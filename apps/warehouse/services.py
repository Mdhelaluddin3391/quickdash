...
def reserve_stock_for_order(order_id, warehouse_id, items):
    ...
    with transaction.atomic():
        ...
        inv_map = {r.sku_id: r for r in inv_rows}
...
def create_picking_task_from_reservation(order_id, warehouse_id, allocations):
    with transaction.atomic():
        pt = PickingTask.objects.create(
            order_id=str(order_id), warehouse_id=warehouse_id, status="pending"
        )
        for sku_id, allocs in allocations.items():
            for a in allocs:
                PickItem.objects.create(
                    task=pt, sku_id=sku_id, bin_id=a["bin_id"], qty=a["allocated"]
                )
        return pt
...
def scan_pick(task_id, pick_item_id, bin_code, sku_code, qty_scanned, scanned_by_user):
    with transaction.atomic():
        ...
        inv.reserved_qty = F("reserved_qty") - take
        ...
        task = pi.task
        all_items = task.items.all()
        if all(it.picked_qty >= it.qty for it in all_items):
        ...
        # notify front-end to retry skipped items
...
def resolve_skip_as_shortpick(pick_skip: PickSkip, created_by_user=None, note=""):
    with transaction.atomic():
        spi = ShortPickIncident.objects.create(
            pick_item=pick_skip.pick_item, created_by=created_by_user, note=note
        )
...
def admin_fulfillment_cancel(pick_item: PickItem, admin_user, reason=""):
    ...
    with transaction.atomic():
        ...
        fc = FulfillmentCancel.objects.create(
            pick_item=pick_item, admin=admin_user, reason=reason
        )

        remaining = pick_item.qty - pick_item.picked_qty
        if remaining > 0:
            bi = BinInventory.objects.select_for_update().get(
                bin_id=pick_item.bin_id, sku_id=pick_item.sku_id
            )
            ...
            inv = InventoryStock.objects.select_for_update().get(
                warehouse_id=pick_item.task.warehouse_id, sku_id=pick_item.sku_id
            )
            inv.available_qty = F("available_qty") + remaining
            inv.reserved_qty = F("reserved_qty") - remaining
...
def create_packing_task_from_picking(picking_task_id):
    with transaction.atomic():
        ...
        pack = PackingTask.objects.create(picking_task=pt, status="pending")

        for it in pt.items.all():
            PackingItem.objects.create(
                packing_task=pack, sku=it.sku, qty=it.picked_qty
            )
...
def create_grn_and_putaway(warehouse_id, grn_no, received_items, created_by=None, metadata=None):
    with transaction.atomic():
        ...
        task = PutawayTask.objects.create(grn=grn, warehouse_id=warehouse_id)
        ...
        for it in received_items:
            ...
            if preferred:
                ...
                sub = BinInventory.objects.filter(
                    bin_id=OuterRef("pk")
                ).values("bin").annotate(s=Sum("qty")).values("s")[:1]
                bins = bins.annotate(total_qty=Subquery(sub, output_field=models.IntegerField()))
...
def place_putaway_item(putaway_task_id, putaway_item_id, bin_id, qty, performed_by):
    with transaction.atomic():
        pai = PutawayItem.objects.select_for_update().get(
            pk=putaway_item_id, putaway_task_id=putaway_task_id
        )
        ...
        inv.available_qty = F("available_qty") + qty
        ...
        pai.placed_qty = F("placed_qty") + qty
...
def create_cycle_count(warehouse_id, created_by, sample_bins=None):
    with transaction.atomic():
        task = CycleCountTask.objects.create(
            warehouse_id=warehouse_id, created_by=created_by, status="pending"
        )

        bins = Bin.objects.filter(shelf__aisle__zone__warehouse_id=warehouse_id)
        if sample_bins:
...
def record_cycle_count_item(cycle_task_id, bin_id, sku_id, counted_qty, counted_by):
    with transaction.atomic():
        item = CycleCountItem.objects.select_for_update().get(
            cycle_task_id=cycle_task_id, bin_id=bin_id, sku_id=sku_id
        )
        ...
        if counted_qty != item.expected_qty:
            ...
            bi = BinInventory.objects.select_for_update().get(bin_id=bin_id, sku_id=sku_id)
            bi.qty = F("qty") + delta
            bi.save(update_fields=["qty"])

            inv = InventoryStock.objects.select_for_update().get(
                warehouse_id=item.cycle_task.warehouse_id, sku_id=sku_id
            )
            inv.available_qty = F("available_qty") + delta
            inv.save(update_fields=["available_qty"])
            ...
            StockMovement.objects.create(
                ...
                reference_id=str(item.cycle_task.id),
            )
